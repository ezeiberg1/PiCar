from mod8_func import movingAvg
import time
import matplotlib
matplotlib.use('Pdf')
...
while h < len(difference):
    if arrAvg[h] > threshold  and upDown == 1: #if first time past dark threshold
        transition = transition + 1
        upDown = -1
        lightDarkArr[h] = 1
    elif arrAvg[h] < -threshold  and upDown == -1: #if first time past light threshold
        transition = transition + 1
        upDown = 1
        lightDarkArr[h] = -1
    else:
...
#calculating rpm
print(transition)
rps = (transition/4)/(times[len(times)-1]-times[100])
print(rps)
