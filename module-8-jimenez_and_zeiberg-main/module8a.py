...
lastTransitionTime = 0 #Time that the last transition in the 5 transition count occured
...
while time_start+tim > cur_time: #Start collecting data
  if time_start + sampleTime*samplePosition< cur_time: #if enough time between samples
    ...
    if difference[samplePosition-1] > .2*maxval  and upDown == 1: #if first time past dark threshold
    ...
    elif difference[samplePosition-1] < -.2*maxval  and upDown == -1: #if first time past light threshold
      transitionCount = transitionCount+1 #increment total transition count
      transitions[samplePosition] = 1  #record that a transition happened here
    ...
    if time_start+ calc*speedPosition< cur_time: #if enough time has passed since last speed calculation
      ...
      if firstTransitionTime != 0 and transitionTrack>1:
         ...
         rps = 1/rps
         #rps = (transitionTrack/5)/(firstTransitionTime - lastTransitionTime) #calculating rps
         #print('calc')
    rpsArray[samplePosition] = rps #recording most recent rps
...
with open('data_PWM_50_velocity.txt','w') as data:
