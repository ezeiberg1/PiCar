...
parser.add_argument('--tim', action='store', type=int, default = 9,
...
cycle = args.pwm
rpsEnter = args.rps
desiredPWM = (args.rps-1.0798)/.1044
kp = args.kp
ki = args.ki
...
while time_start+tim > cur_time: #Start collecting data
  ...
  if time_start + sampleTime*samplePosition< cur_time: #if enough time between samples
    ...
    checkCount = 0
    velCount = 0
    if time_start+ calc*velCount< cur_time: #if enough time has passed since last speed calculation
      transitionTrack = 0
      transitionPosition = samplePosition #position in transition array
      lookFor = 1
      firstTransitionTime = 0;
      lastTransitionTime = 0;
      while transitionTrack < 5 and transitionPosition >= 0 and checkCount < 80:
      ...
      elif firstTransitionTime != 0 and transitionTrack>1:
...
with open('M10_car_noload_3rps.txt','w') as data:
 data.write(f'{sampleTime}\n')
...
file    = open('M10_car_noload_3rps.txt', 'r')
...
plt.ylabel('Velocity')
plt.xlabel('time - sec')
plt.savefig("M10_plot_noload.png")
...
plt.grid(True)
plt.xlabel("freq - Hz")
plt.savefig("M10_fft_noload.png")
